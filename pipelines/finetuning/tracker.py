...
class FineTuneTracker:
    ...
    def start_training_job(self, job_id: UUID, base_model: str, pairs: List[Dict[str, Any]]) -> str:
        with mlflow.start_run(run_name=f"finetune-{job_id}") as run:
            ...
            avg_quality = sum([p.get("quality_score", 0.0) for p in pairs]) / len(pairs) if pairs else 0.0
    ...
    def log_job_completion(self, run_id: str, job_id: UUID, provider_job_id: str, metrics: Dict[str, Any]):
