...
class PipelineOptimizer:
    ...
    async def get_suggestions(self, pipeline_id: UUID) -> List[Dict[str, str]]:
        ...
        if not stats:
            return [{"type": "info", "message": "Not enough evaluation data to generate suggestions."}]
        ...
        if not suggestions:
            suggestions.append({"type": "success", "message": "All metrics are looking great! No configuration changes suggested at this time."})
