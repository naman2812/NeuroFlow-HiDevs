...
class TimeoutManager:
    ...
    @classmethod
    async def _update_latency(cls, task_type: str, latency: float):
            ...
            if len(latencies) >= 10:  # Need at least a few samples to be meaningful
                ...
                if history:
                    ...
                    if now - oldest_time > 900 and p95 > oldest_p95 * 1.5:
                        logger.warning(f"Increasing p95 latency trend detected for {task_type}! Old p95: {oldest_p95:.2f}s, Current p95: {p95:.2f}s")
    ...
    @classmethod
    async def run(cls, task_type: str, coro):
