...
class URLIngestRequest(BaseModel):
    url: str = Field(
        ...,
        description="The fully qualified HTTP/HTTPS URL to scrape and ingest into the knowledge base.",
        example="https://en.wikipedia.org/wiki/Artificial_intelligence",
        json_schema_extra={"example": "https://en.wikipedia.org/wiki/Artificial_intelligence"}
    )
    pipeline_id: str | None = Field(None, description="Optional pipeline ID to link the document to a specific pipeline.")
...
class IngestResponse(BaseModel):
    ...
    duplicate: bool = Field(..., description="True if the document was already ingested based on content hash.")
    warning: str | None = Field(None, description="Any backpressure warnings.")
    estimated_wait_minutes: int | None = Field(None, description="Estimated queue wait time if under heavy load.")
...
@router.post(
    ...
    description="Uploads a file (PDF, Docx, Text, Image) to be ingested into the knowledge base. This endpoint adds the document to an asynchronous queue for chunking and embedding. **Performance notes**: Rate limited to 10 requests per hour. Returns a 503 if the system is experiencing heavy backpressure. **Errors**: Triggers a 400 error for unsupported file types or files larger than 100MB.",
    response_description="A JSON object containing the generated document_id, queue status, and deduplication flag.",
...
async def ingest_file(
    ...
    if bp_warning and bp_warning["status_code"] == 503:
        return JSONResponse(status_code=503, content={"error": bp_warning["error"], "queue_depth": bp_warning["queue_depth"], "retry_after": bp_warning["retry_after"]})
    ...
    async with db_pool.acquire() as conn:
        ...
        if existing:
            return IngestResponse(document_id=str(existing["id"]), status=existing["status"], duplicate=True)
        ...
        await conn.execute(
            "INSERT INTO documents (id, filename, source_type, content_hash, status) VALUES ($1, $2, $3, $4, 'queued')",
...
@router.post(
    ...
    description="Provides a URL to be ingested into the knowledge base via a JSON payload. This endpoint adds the document to an asynchronous queue for crawling and embedding. **Errors**: Triggers a 400 error for Server-Side Request Forgery (SSRF) attempts on local IP addresses.",
    response_description="A JSON object containing the generated document_id, queue status, and deduplication flag.",
    response_model=IngestResponse
)
async def ingest_url(
    ...
    await redis.enqueue_job("process_document", doc_id, url, "url", _queue_name="queue:ingest")
...
@router.get(
    ...
    description="Poll this endpoint to check the asynchronous processing status of an ingested document. Use this to determine when a document is fully chunked, embedded, and ready for querying (status will be `complete`). **Errors**: Triggers a 400 for an invalid UUID format, or a 404 if the document ID does not exist in the database.",
    response_description="A JSON object containing the current processing status, chunk count, and extracted document metadata."
)
