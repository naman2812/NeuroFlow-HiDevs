...
async def main():
    ...
    async with httpx.AsyncClient(timeout=60.0) as client:
        ...
        logger.info("Checking 1: PipelineConfig rejects unknown keys...")
        bad_config = {
            "config": {
        ...
        if res.status_code == 422:
            logger.info("[SUCCESS] Unknown keys rejected with 422")
        else:
            logger.info(f"[FAILED] Expected 422, got {res.status_code}")
            
        # 2. Pipeline updates create new versions
        logger.info("Checking 2: Pipeline updates create new versions...")
        ...
        if v2 > v1:
            logger.info(f"[SUCCESS] Version incremented from {v1} to {v2}")
        else:
            logger.info(f"[FAILED] Version did not increment. v1={v1}, v2={v2}")
            
        # 3. POST /pipelines/compare runs in parallel
        logger.info("Checking 3: Compare runs in parallel...")
        ...
        if res_compare.status_code == 200:
            ...
            logger.info(f"[SUCCESS] Compare completed in {total_compare_time:.2f}s returning A and B.")
            a_run_id = a_data.get("run_id")
            if not a_run_id:
                logger.info("Note: run_id not in pipeline_a response (perhaps it errored out gracefully)")
        else:
            logger.info(f"[FAILED] Compare returned {res_compare.status_code}")
            
        # 4 & 5. Check Analytics and DB records
        logger.info("Checking 4 & 5: Analytics and pipeline_runs...")
        ...
        if res_analytics.status_code == 200:
            ...
            if "p95_retrieval_latency" in analytics:
                logger.info("[SUCCESS] Analytics returns p95 latency correctly.")
            else:
                logger.info("[FAILED] Analytics missing p95_retrieval_latency.")
        ...
        if res_runs.status_code == 200:
            ...
            if runs and all('pipeline_version' in r for r in runs):
                logger.info(f"[SUCCESS] pipeline_runs records pipeline_version (Found in {len(runs)} runs).")
            else:
                logger.info("[FAILED] pipeline_runs missing pipeline_version.")
