...
class EvaluationJudge:
    ...
    async def _run_metrics(self, query: str, generation: str, context: str, chunks: list, temperature: float = None):
        ...
        precision_task = evaluate_context_precision(query, chunks, generation, self.client, **kwargs)
    ...
    async def evaluate_run(self, run_id: str):
        ...
        with tracer.start_as_current_span("evaluation.judge") as span:
            ...
            tasks = [self._run_metrics(query, generation, context, chunks, temperature=0.7) for _ in range(3)]
            ...
            async with self.db_pool.acquire() as conn:
                await conn.execute(
                    ...
                    INSERT INTO evaluations (
                        run_id, faithfulness, answer_relevance, context_precision, context_recall, overall_score, judge_model, metadata
                    ) VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
                    """,
                    UUID(run_id), avg_faithfulness, avg_relevance, avg_precision, avg_recall, avg_overall, "routed_evaluator", json.dumps(metadata)
                )
                ...
                if avg_overall > 0.8:
                    await conn.execute(
                        ...
                        """,
                        UUID(run_id), prompt, query, generation, avg_overall
                    )
                    ...
                    await conn.execute(
                        ...
                        SET metadata = jsonb_set(COALESCE(metadata, '{}'::jsonb), '{is_training_candidate}', 'true'::jsonb)
                        WHERE id = $1
                        """,
                        UUID(run_id)
                    )
